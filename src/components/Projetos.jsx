  ...
  return (
    <section id="projetos" style={{ background: 'rgba(15, 23, 42, 0.5)' }}>
      <Container>
        ...
        >
          <h2 style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: '1rem', color: 'var(--primary)' }}>Meus Projetos</h2>
              ...
              >
                <Card className="glass text-white h-100" style={{ borderRadius: '15px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }}>
                  ...
                  <Card.Body className="d-flex flex-column p-4">
                    <Card.Title style={{ fontWeight: 700, fontSize: '1.4rem', color: 'var(--primary)' }}>{project.title}</Card.Title>
                    ...
                    <div className="mt-auto d-flex flex-column gap-2">
                      <Button onClick={() => handleExpand(project.id)} variant="outline-info" style={{ width: '100%', borderRadius: '10px' }}>
                        {expandedId === project.id ? 'Ocultar detalhes' : 'Ver mais sobre'}
                      </Button>
              ...
              return (
                <div key={`detail-${p.id}`} className="text-white">
                  <div className="d-flex flex-column align-items-center mb-4 text-center">
                    <h3 style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--primary)' }}>{p.title}</h3>
                    <Badge className="fs-6 mt-2" style={{ backgroundColor: '#2e8b57', padding: '8px 15px', borderRadius: '15px' }}>{p.nature || 'Projeto'}</Badge>
                  </div>
                  ...
                  <Row className="gy-5">
                    ...
                    <Col md={6} className="d-flex flex-column pe-md-4">
                      <h4 style={{ color: 'var(--secondary)', fontWeight: 700 }} className="mb-3">Sobre o Projeto</h4>
                      <div style={{ color: 'var(--text-light)', lineHeight: '1.8', textAlign: 'justify' }} dangerouslySetInnerHTML={{ __html: p.fullDescription }} />
                      {p.descImage && (
                    ...
                    <Col md={6} className="d-flex flex-column border-start border-secondary ps-md-4" style={{ borderColor: 'rgba(255,255,255,0.1) !important' }}>
                      <h4 style={{ color: 'var(--secondary)', fontWeight: 700 }} className="mb-3">Desenvolvimento</h4>
                      <div style={{ color: 'var(--text-light)', lineHeight: '1.8', textAlign: 'justify' }} dangerouslySetInnerHTML={{ __html: p.developmentProcess }} />
                      {p.devImage && (
                        <div className="mt-3 mt-auto align-self-center" style={{ width: '90%', borderRadius: '15px', overflow: 'hidden' }}>
                  ...
                  <div className="d-flex justify-content-center mt-5 gap-3">
                    ...
                    >
                      <FaExternalLinkAlt /> Ver Demonstração
                    </Button>
                  </div>
                </div>
              );
